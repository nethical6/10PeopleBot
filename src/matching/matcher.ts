...
import { ChannelType, EmbedBuilder, PermissionOverwriteManager, PermissionOverwrites, PermissionsBitField } from "discord.js";
...
export const Matcher = async (bot:Bot) =>{ 
    ...
    for (const waiter of allWaiters) {
        ...
        for (const group of existingGroups) {
            ...
            console.log(`Average match for group ${group.id} with waiter ${waiter}: ${averageGroupMatch}`);
        ...
        for (const otherWaiter of allWaiters) {
            if (waiter === otherWaiter || matchedUserIds.includes(otherWaiter)) continue; 
            ...
            if (match >= 0.3) {
                const topInterest = new Set([...waiterInterestSet].filter(x => otherInterestSet.has(x)))
                // Create a new group with matched users
                const interestsArr = Array.from(topInterest);
                const randomInterest = interestsArr[Math.floor(Math.random() * interestsArr.length)] || "Group";
                const groupName = `${randomInterest}-${Math.floor(Math.random() * 10000)}`;
                const members:string[] = [waiter, otherWaiter];
                const channelId = await CreateGroup(bot, groupName, members,match*100);
                ...
                console.log(`Matched ${waiter} and ${otherWaiter} into group ${groupName}`);
                break; // Exit inner loop once matched
            }
        }
...
export const CreateGroup = async (bot: Bot, groupName: string, members: string[], matchPercentage: number): Promise<string | null> => {
    ...
    const validMembers = memberObjects.filter((member): member is NonNullable<typeof member> => member !== null);
    ...
    if (channel) {
        ...
        if (channel.isTextBased()) {
            const welcomeEmbed = new EmbedBuilder()
                ...
                .setDescription(`Welcome! We've matched you based on your similar interests. More people might join soon!\n\n<@${members.join('>, <@')}>`)
...
export const AddToGroup = async (bot: Bot, groupId: string, userId: string, matchPercentage:number) => {
    const guild = bot.client.guilds.cache.get(GUILD_ID);
    await guild?.fetch()
    ...
    const welcomeEmbed = new EmbedBuilder()
                ...
                .setDescription(`Say hi to our new member <@${userId}>! We matched them based on their interests with this group.`)
                .addFields({ name: "Match Rate", value: matchPercentage.toFixed(2) + "%" })
                .setFooter({ text: 'Happy chatting!' })
                .setColor('#5865F2')
                .setTimestamp();
